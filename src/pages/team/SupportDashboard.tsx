  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-2">
        <LifeBuoy size={18} className="text-neutral-400" />
        <h1 className="font-display text-title-1 font-bold text-neutral-900">Support queue</h1>
      </div>
      <p className="mt-1 text-callout text-neutral-600">Resolve buyer queries and NOC requests for {membership?.builderName}&rsquo;s projects.</p>
      ...
      {projects.length === 0 ? (
        ...
        <>
          ...
          <div className="mt-6">{selected && <RequestsTab projectId={selected} />}</div>
        </>
      )}
    </div>
  )
